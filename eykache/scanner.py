...
class EventScanner:
    ...
    async def scan(
        ...
        """Perform a PlotChange event scan.

        Assumes all PlotChanges in the database are valid before start_block (no forks sneaked in).
        ...
        while current_block <= end_block:
            ...
            logger.debug(
                "Scanning plot changes for blocks: %d - %d, chunk size %d, last chunk scan took %f, last logs found %d",
                current_block,
                estimated_end_block,
                chunk_size,
                last_scan_duration,
...
class DatabasedState(EventScannerState):
    ...
    def save(self):
        """Save everything we have scanned so far in db and file."""
    ...
    def end_chunk(self, block_number):
        ...
        # Save the database file for every 15 seconds
